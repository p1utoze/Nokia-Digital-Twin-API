...
async def test_sso_provider(
    *, sso: SSOBase = Depends(deps.get_generic_sso)
    ...
    """
    Test if the SSO provider is working correctly
    """
    print(sso.redirect_uri)
    return {"msg": "SSO provider is working correctly"}
...
def create_sso_user(db: Session, provider: models.Provider, openid_user: OpenID, *, country: str = None) -> str:
    ...
    token = security.create_sso_confirmation_token(user.id, user.sso_confirmation_code)
    return token
